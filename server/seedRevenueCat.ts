...
const PROJECT_NAME = "Knitto";

const PRODUCT_IDENTIFIER = "knitto_premium_monthly";
const PLAY_STORE_PRODUCT_IDENTIFIER = "knitto_premium_monthly:monthly";

const PRODUCT_DISPLAY_NAME = "Knitto Premium Monthly";
const PRODUCT_USER_FACING_TITLE = "Knitto Premium";
const PRODUCT_DURATION = "P1M";

const APP_STORE_APP_NAME = "Knitto iOS";
const APP_STORE_BUNDLE_ID = "com.ullogpinner";
const PLAY_STORE_APP_NAME = "Knitto Android";
const PLAY_STORE_PACKAGE_NAME = "com.ullogpinner";

const ENTITLEMENT_IDENTIFIER = "premium";
const ENTITLEMENT_DISPLAY_NAME = "Knitto Premium";
...
const PRODUCT_PRICES = [
  ...
  { amount_micros: 69000000, currency: "NOK" },
];
...
async function seedRevenueCat() {
  ...
  if (attachPackageError) {
    if (
      ...
      console.log("Skipping package attach: already has a product for this store");
    } else {
  ...
  console.log("EXPO_PUBLIC_REVENUECAT_TEST_API_KEY:", testStoreApiKeys?.items.map((k) => k.key).join(", ") ?? "N/A");
  console.log("EXPO_PUBLIC_REVENUECAT_IOS_API_KEY:", appStoreApiKeys?.items.map((k) => k.key).join(", ") ?? "N/A");
  console.log("EXPO_PUBLIC_REVENUECAT_ANDROID_API_KEY:", playStoreApiKeys?.items.map((k) => k.key).join(", ") ?? "N/A");
  console.log("====================\n");
}
